...
//Act: sut.Something internally use IDateTimeWrapper.Now and returns the current datetime.
var result = sut.Something(request);

//Assert: result should match the mocked time value
...
- To set up a mock, first ensure an interface (or virtual member) exists for the dependency.
